...
# def create_ps_sca_year(ps_sca_tif, basin, chm_mask, name):
#     # Making no snow 0
#     # Planet scope dataframe
#     ps_sca_df = []

#     for file in ps_sca_tif:
#         #extracting filename & date
#         filename = os.path.basename(file)
#         date = file.split("/")[-1].split(".")[0].split("_")[0] # LOOK INTO THIS
#         # Open SCA raster
#         ps_sca = rioxarray.open_rasterio(file, all_touched=False,drop=True, masked=True)
#         raster_crs = ps_sca.rio.crs
        

#         # Masks : basin, RGI, and WBD
...
#         # Clip SCA raster to basin geometry by assigning no data pixels to 0
#         ps_sca.values=np.where(np.isnan(ps_sca), 0, ps_sca)
#         ps_sca = ps_sca.rio.clip(basin.geometry.values, crs=basin.crs, drop=True)
        
#         ps_sca = ps_sca.rio.clip(rgi_mask.geometry.values,invert=True) #tetsing
#         ps_sca = ps_sca.rio.clip(wbd_mask.geometry.values,invert=True) #tetsing
        
#         # Total pixel count
#         #cnt = PS_sca.values.size
#         # Snow absence or pixel outside of domain
#         five_cnt = (ps_sca.values == 0).sum()
#         # Classified snow presence
#         one_cnt = (ps_sca.values == 1).sum()
...
#         two_cnt = (ps_sca.values == 2).sum()
#         # Total pixel count
#         cnt = five_cnt + one_cnt + two_cnt
#         #snow presence percentage
#         one_pct = one_cnt/cnt
#         #snow absence percentage
#         five_pct = five_cnt/cnt
...
#         ps_sca_chm = xarray.DataArray(ps_sca_chm, dims=ps_sca.dims, coords={dim: ps_sca.coords[dim] for dim in ps_sca.dims})
        
#         chm_five_cnt = (ps_sca_chm.values == 0).sum()
#         chm_one_cnt = (ps_sca_chm.values == 1).sum()
#         chm_two_cnt = (ps_sca_chm.values == 2).sum()
...
#     ps_sca_year = pd.DataFrame(ps_sca_df)
#     ps_sca_year = ps_sca_year[ps_sca_year['f_unobs']<.02]
#     ps_sca_year = ps_sca_year.sort_values("date", ascending=True)
#     ps_sca_year['date'] =  pd.to_datetime(ps_sca_year['date'], format='%Y%m%d')
#     #ps_sca_year['fsca_adj'] = ps_sca_year['fsca_obs']/(1-fcan)  
...
def validation_tif_binary(aso_tif, basin, year, name):
    for file in aso_tif:
        if year in file:
